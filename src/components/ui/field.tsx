...
const fieldVariants = cva(
    "group/field flex w-full gap-2 data-[invalid=true]:text-destructive data-[valid=true]:text-success",
    {
        variants: {
            orientation: {
...
function FieldSuccess({
    ...
    messages?: Array<{ message?: string } | undefined>;
}) {
    const content = useMemo(() => {
        ...
        const uniqueMessages = [...new Map(messages.map((msg) => [msg?.message, msg])).values()];

        if (uniqueMessages?.length === 1) {
            return uniqueMessages[0]?.message;
        }

        return (
            <ul className="ml-4 flex list-disc flex-col gap-1">
                {uniqueMessages.map(
                    (msg, index) => msg?.message && <li key={index}>{msg.message}</li>,
                )}
            </ul>
        );
    }, [children, messages]);
    ...
    return (
        <div
            ...
            className={cn("text-success text-sm font-normal", className)}
            {...props}
        >
            {content}
        </div>
